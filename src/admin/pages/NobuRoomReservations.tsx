...
function reservationsCacheKey(dateFilter: string, search: string) {
  return `reservations:nobu-room:${dateFilter}:${search}`
}
...
export default function NobuRoomReservations() {
  ...
  const cachedReservations = getPageCache<NobuRoomReservation[]>(initialCacheKey)
  const [reservations, setReservations] = useState<NobuRoomReservation[]>(cachedReservations ?? [])
  ...
  const load = useCallback(async (opts: { silent?: boolean } = {}) => {
    ...
    const cached = getPageCache<NobuRoomReservation[]>(cacheKey)
    if (!opts.silent) {
      if (cached) {
    ...
    try {
      ...
      setPageCache<NobuRoomReservation[]>(cacheKey, data.reservations)
    } catch (err: any) {
