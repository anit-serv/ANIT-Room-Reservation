      ...
      if (editingId) {
        ...
        if (shouldSaveFav) {
          fetch('/api/favorites', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${profile.getAccessToken()}` },
            body: JSON.stringify({ name: savedName, kobuTimeSlot: `${modalStart}-${modalEnd}` }),
          }).then(r => r.ok ? r.json() : null)
            .then(d => { if (d?.id) setFavorites(prev => [...prev, { id: d.id, name: savedName, nobuTimeSlot: null, kobuTimeSlot: `${modalStart}-${modalEnd}`, nobuRoomTimeSlot: null }]) })
            .catch(() => {})
        }
        ...
        return (
          <div className={`fixed inset-0 z-50 flex flex-col justify-end ${modalClosing ? 'animate-fade-out' : 'animate-fade-in'} ${sheetPeeking ? 'pointer-events-none' : ''}`}>
            ...
            >
              ...
              <div className="mb-3">
                {!editingId && (
                  <FavoritePicker favorites={favorites} onSelect={(favId, name, _nobuTimeSlot, kobuTimeSlot) => {
                    setBandName(name); setSelectedFavId(favId); setSaveAsFavChecked(false); applyFavoriteTimeSlot(kobuTimeSlot)
                  }} />
                )}
                <input
