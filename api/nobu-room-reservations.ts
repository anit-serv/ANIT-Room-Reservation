...
async function handleCreate(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!isAvailable) {
      return res.status(400).json({ error: 'この日は農部室を予約できません' })
    }
    ...
    const existingSnap = await db.collection('nobu_room_reservations').where('date', '==', date).get()
    ...
    await db.collection('nobu_room_reservations').add({
      userId,
...
async function handleMy(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const today = nowJST().toISOString().slice(0, 10)
    const snap  = await db.collection('nobu_room_reservations').where('userId', '==', userId).get()
...
async function handleAll(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const snap = await db.collection('nobu_room_reservations')
...
async function handleDelete(req: VercelRequest, res: VercelResponse, docId: string) {
  try {
    const { userId } = await verifyLineToken(req.headers.authorization)
    const docRef = db.collection('nobu_room_reservations').doc(docId)
...
async function handleModify(req: VercelRequest, res: VercelResponse, docId: string) {
  try {
    ...
    const existingSnap = await db.collection('nobu_room_reservations').where('date', '==', data.date).get()
